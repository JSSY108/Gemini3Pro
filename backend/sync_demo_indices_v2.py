...
# These IDs are from scanned_sources
targets = [
    {
...
# Also sync reliability_metrics.segments
new_metric_segments = []
for support in new_supports:
    segment_text = support['segment']['text']
    # Create SourceAudit objects for this segment
    # We'll just bridge to the scanned sources
    sources_audit = []
    for i, chunk_idx in enumerate(support['grounding_chunk_indices']):
        source_id = chunk_idx + 1
        # Find domain from scanned_sources
...
print(f"Sync complete. Created {len(new_supports)} segments.")
